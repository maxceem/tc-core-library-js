...
module.exports = function (options) {
  ...
  let secret = _.get(options, "AUTH_SECRET", config.get('AUTH_SECRET'))
  let validIssuers = JSON.parse(_.get(options, "VALID_ISSUERS", config.get('VALID_ISSUERS')))
  ...
  let jwksClient = jwksRSA({
    ...
    jwksUri: config.get("JWKS_URI") + '.well-known/jwks.json'
  })
  ...
  return function (req, res, next) {
    ...
    if (token) {
      ...
      if (decodedToken && decodedToken.header){
        ...
        // to be removed
        req.authUser = decodedToken.payload;
        req.authUser.userId = Number(req.authUser.userId);
        return next();
